...
![Call Diagram](Img/Fig.1.1.InteropDiagram.svg)
...
2. For the Active solution platform:
 • Choose your platform (in the example code ARM64)
 • If missing → New… → ARM64 (it should be your architecture)
3. Ensure:
 • NativeDeviceLib → ARM64 (it should be your architecture)

 **Note:** If this is wrong, interop will compile but fail at runtime.

#### 1.2.3 Project structure (best practice)

Inside NativeDeviceLib, create source and header files in the dedicated directories:
...
The header file should contain a secondary-include protection macro and an export function definition.
...
- extern "C" instead of "C++" → stable ABI (no name mangling)
...
#### 1.3.2 Force your target architecture, not Any CPU (in the example, ARM64 is used)

 1. Right-click ManagedClient → Properties
 2. Go to Build
 3. Platform target:
 • Yours (in the example, ARM64 is used)
...
The easiest way is to copy it manually, or, even better, use a post-build step script to copy it.
